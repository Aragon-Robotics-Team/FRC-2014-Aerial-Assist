...
public:
	...
	void Autonomous(void)
	{
		...
		AxisCamera &camera = AxisCamera::GetInstance("10.8.40.11");	//To use the Axis camera uncomment this line
		...
		while (IsAutonomous() && IsEnabled()) {
			...
			if(reports->size() > 0)
			{
				...
				for (unsigned int i = 0; i < MAX_PARTICLES && i < reports->size(); i++) {
					...
					{
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "particle: %d  is a Horizontal Target centerX: %d  centerY: %d \n", i, report->center_mass_x, report->center_mass_y);
						driverStation->UpdateLCD();
						horizontalTargets[horizontalTargetCount++] = i; //Add particle to target array and increment count
					} else if (scoreCompare(scores[i], true)) {
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "particle: %d  is a Vertical Target centerX: %d  centerY: %d \n", i, report->center_mass_x, report->center_mass_y);
						driverStation->UpdateLCD();
						verticalTargets[verticalTargetCount++] = i;  //Add particle to target array and increment count
					} else {
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "particle: %d  is not a Target centerX: %d  centerY: %d \n", i, report->center_mass_x, report->center_mass_y);
					}
					driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "Scores rect: %f  ARvert: %f \n", scores[i].rectangularity, scores[i].aspectRatioVertical);
					driverStation->UpdateLCD();
					driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "ARhoriz: %f  \n", scores[i].aspectRatioHorizontal);	
					driverStation->UpdateLCD();
				}
				...
				if(verticalTargetCount > 0)
				{
					...
					if(target.Hot)
					{
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "Hot target located \n");
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "Distance: %f \n", distance);
					} else {
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "No hot target present \n");
						driverStation->PrintfLine(DriverStationLCD::kUser_Line1, "Distance: %f \n", distance);
					}
				}
			}
